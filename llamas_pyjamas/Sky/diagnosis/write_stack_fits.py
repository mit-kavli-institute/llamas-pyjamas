"""Write per-channel DS9 stacks: pedestal8_stack_{blue,green,red}_{off,on,diff}.fits (common grid
per channel; blue/red use the registered green-sibling FIBERWCS)."""
...
def load(v,chan):
    ...
    for ei,f in enumerate(sorted(glob.glob(f'{ND}/reduced_ped_{v}/extractions/*_RSS_{chan}.fits'))):
        with fits.open(f) as h:
            ...
            fm=h['FIBERWCS'].data if 'FIBERWCS' in [x.name for x in h] else None
        if fm is None:
            with fits.open(f.replace(f'_RSS_{chan}','_RSS_green')) as hg:
...
for chan in ('blue','green','red'):
    ...
    for v in ('off','on'):
        ...
        im=coadd_image(FibreTable(ra=A[0],dec=A[1],value=A[3],var=A[4],
            ...
            channel=np.array([chan]*A[0].size),npix=A[5]),units='flux',weighting='ivar',center=ctr)
        ims[v]=im
        out=f'{ND}/pedestal8_stack_{chan}_{v}.fits'; im.write(out); print('wrote',out)
    a,b=ims['off'].data,ims['on'].data
    ny,nx=min(a.shape[0],b.shape[0]),min(a.shape[1],b.shape[1])
    hdr=ims['off'].wcs.to_header(); hdr['COMMENT']=f'{chan}: pedestal OFF minus template ON'
    fits.PrimaryHDU((a[:ny,:nx]-b[:ny,:nx]).astype(np.float32),header=hdr).writeto(
        f'{ND}/pedestal8_stack_{chan}_diff.fits',overwrite=True)
    print('wrote',f'{ND}/pedestal8_stack_{chan}_diff.fits')
# remove the old untagged (green-only) files to avoid confusion
for v in ('off','on','diff'):
    p=f'{ND}/pedestal8_stack_{v}.fits'
    if os.path.exists(p): os.remove(p); print('removed old',p)
print('DONE')
